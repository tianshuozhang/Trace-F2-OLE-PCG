...
// OT 发送方 (P1)
void sender_OT(NetIO* io, vector<uint8_t>& A, size_t c, size_t t, vector<uint8_t>& R) {
...
// OT 接收方 (P2)
void receiver_OT(NetIO* io, vector<uint8_t>& B, size_t c, size_t t, vector<uint8_t>& S) {
    uint32_t length = t * c;
    const uint32_t size  = length*length;
...
// 线程函数
void run_sender(size_t c, size_t t,vector<uint8_t>&A,vector<uint8_t> &R) {
...
void run_receiver(size_t c, size_t t,vector<uint8_t>&B,vector<uint8_t>&S) {
